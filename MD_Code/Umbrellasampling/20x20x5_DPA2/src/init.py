...
def read_input():
    ...
    idx = 0
    # 1. Ensemble
    if linenomalize(lines[idx]) == 'ensemble':
        idx += 1
    ...
    # 2. Number of steps
    if linenomalize(lines[idx]) == 'number of steps':
        idx += 1
    ...
    # 3. Output steps
    if linenomalize(lines[idx]) == 'output steps':
        idx += 1
    ...
    # 4. Time step(ps)
    if linenomalize(lines[idx]) == 'time step(ps)':
        idx += 1
    ...
    # 5. Temperature(K)
    if linenomalize(lines[idx]) == 'temperature(k)':
        idx += 1
    ...
    if linenomalize(lines[idx]) == 'couple frequency(cm-1)':
        idx += 1
    ...
    # 7. Lattice constant
    if linenomalize(lines[idx]) == 'lattice constant':
        idx += 1
    ...
    # 8. Super Cell
    if linenomalize(lines[idx]) == 'super cell':
        idx += 1
        # 一行包含两个整数
    ...
    if linenomalize(lines[idx]).startswith('number'):
        idx += 1
    ...
    header_normalized = ' '.join(lines[idx].split()).lower()
    #if lines[idx] == 'Elem   Mass       X         Y        Z':
    if header_normalized == 'elem mass x y z':
        idx += 1
    else:
    ...
    if linenomalize(lines[idx]).startswith('upper bound'):
        idx += 1
    ...
    if linenomalize(lines[idx]).startswith('lower bound'): 
        idx += 1
    ...
    if idx < len(lines) and linenomalize(lines[idx]) == 'umbrella sampling':
        idx += 1

    # 读取 Window index
    if idx < len(lines) and linenomalize(lines[idx]) == 'window index':
        idx += 1
    ...
    if idx < len(lines) and linenomalize(lines[idx]).startswith('equilibrium z0'):
        idx += 1
    ...
    if idx < len(lines) and linenomalize(lines[idx]).startswith('force constant'):
        idx += 1
