subroutine pes_ruFixco(x,e,dedx) !6d-potential energy surface interface
  implicit none
  real*8,intent(in) :: x(3,2)
  real*8,intent(out) :: e,dedx(3,2)!e -- > energy dedx-->gradient

  call co_ru_pes(x,e,1,dedx)
  dedx = -dedx
  
  write(990,'(7f12.5)') x,e !output the fort.990 file
...
subroutine pes_ru12co(x,e,dedx)
  ...
  c(1:3, 1)=[   0.79107d0,   2.43599d0,   0.00000d0 ]!the fixed layer
  c(1:3, 2)=[   3.16424d0,   1.06584d0,   0.00000d0 ]!the fixed layer
  c(1:3, 3)=[   0.79107d0,  -0.30431d0,   0.00000d0 ]!the fixed layer
