...
!   Call constrain()
    ...
    if (Mod(i_step,outstep) == 0) then
      write(uout,"(2x,I14,6x,F12.3,2ES18.8)") i_step,tempnow,Ev/1.d2/96.4853d0,(Ev+Ek)/1.d2/96.4853d0
                                                           !! from 10J/mol to eV
      if (mod(i_step,outstep*10).eq.0) then
         write(utrjtxt,*) natom
...
!         do iii = 1, natom
!            write(utrjtxt,"(2x,A,2x,3E16.6)")
!            element(iii),(Cmat(jjj,iii),jjj=1,3)
!         end do
!      endif
...
!    Output umbrella sampling
      if (Mod(i_step,outstep) == 0) then
        write(uwc,*) natom
        write(uwc,"(1x,f10.4,A7,2x,I15)") Ev/100.d0/96.4853d0    !, ' Step: ', i_step - 1
        do iii = 1, natom   
