!program test
!  implicit none
!  real*8 :: r(6),v,dvdr(6),vco,dvco
!  !! r: bohr, O1-C1, O1-O2, O1-C2, C1-O2, C1-C2, O2-C2
!  !! v: cm-1;  dvdr: cm-1/bohr
!  r=[2.709248d0,10.463631d0,13.177895d0,8.340613d0,11.211281d0,3.161466d0]
!  call pes_ococ(r,v,1,dvdr)
!  print*,v
!  print*,dvdr
!  call potco(r(1),vco,dvco)
!  print*,vco,dvco
!end program

!! 2020/9/5
!! NN input: molecular based fundamental invariants
!! 7-45-45-1
!! 19998 energies, long range points up to 50 Bohr
!! SCF/CVTZ-F12 + Correlation[AE-CCSD(T)-F12a]/CBS[CVDZ-F12,CVTZ-F12]
!! BSSE has not been considered

!! 2020/6/26
!! updated: fitting using exp(-r*ita)
...
!! 2023/4/26
!! updated: new fit of cbs energies

subroutine pes_ococ(r,v,idv,dvdr)
  implicit none
  ...
  if(idv.ne.0 .and. idv.ne.1) stop "idv=0: energy only; idv=1: v + dvdr"

  call pip_ococ(r,p(:,1),idv,dpdr)
  call nnfit_ococ(np,1,p,vp,idv,dvdp)
...
!! 7-45-45-1 fit to 19998 points, RMSE=3.079 cm-1, train 1.027 cm-1, validation 9.238 cm-1
!-#define pesdir "/home/chenjun/coco/3-nnfit/finn-20k-aef12a-cbs/fit-b-cbs-morse/"
!-#define wfile1 "W87.txt"
#define pesdir "./"
#define wfile1 "pes_ococ_w87.txt"
...
subroutine nnfit_ococ(ndim,ntot,r0,vx,idv,dv)
  implicit none
  ...
  character*99,save :: wfile=pesdir//wfile1
