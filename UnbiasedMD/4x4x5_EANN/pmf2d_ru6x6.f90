program main
  ...
  real*8 :: dax,day,dby
  real*8,parameter :: a0=2.7402961914621904d0!2.812256750888d0
  dax=a0*dsqrt(3.d0)/2.d0
  day=-a0/2.d0
  dby=a0
  ...
  enddo
  narg=i-1
  if(narg.ne.9) stop "./pmf.x 300(K) file pmf2 0 1 30 0 1 30"
...
subroutine MapSix_Ru0001(ns,cart,car6)
  ...
  do i=1,ns
     ...
     call one_cell_carbon_pt111(x,carbon)
     cart(1:2,i)=carbon
...
subroutine one_cell_carbon_pt111(x,carbon)
  implicit none
  ...
  carbon=x
  carbon(1) = carbon(1)-0.791170d0
  carbon(2) = carbon(2)+0.304310d0

  do while(carbon(1).gt.dax)
     carbon(1)=carbon(1)-dax
     carbon(2)=carbon(2)-day
  enddo

  do while(carbon(1).lt.0.d0)
     carbon(1)=carbon(1)+dax
     carbon(2)=carbon(2)+day
  enddo

  do while(carbon(2).gt.day)
     carbon(2)=carbon(2)-dby
  enddo

  do while(carbon(2).lt.carbon(1)/sqrt(3.d0))
     carbon(2)=carbon(2)+dby
  enddo
