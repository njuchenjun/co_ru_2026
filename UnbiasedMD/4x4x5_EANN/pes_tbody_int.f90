module nnmod_pes_tbody_int !!@@@@ rename
     integer(kind=4),parameter :: intype=4,retype=8,atomdim=3
     real(kind=retype) :: pi=dacos(-1.d0)

     character(kind=1,len=120),parameter :: pesdir='./pes_coru/tbody-in/pes1.dat'
     integer(kind=intype),parameter :: pesid=8
...
subroutine init_pes_tbody_int !!@@@@ rename
    implicit none
...
subroutine evaluate_pes_tbody_int(ifrac,iforce,natoms,symbols,coord,e,f) !!@@@@ rename
      implicit none
...
subroutine deallocate_pes_tbody_int !!@@@@ rename
     implicit none
       deallocate(nwave)
       deallocate(inta)
       deallocate(rs)
